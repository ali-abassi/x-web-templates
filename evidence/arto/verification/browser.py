...
R=Path('/Users/aliabassi/Documents/ChatGPT/AI Designer/x-web-templates');E=R/'evidence/arto';P=['agent-browser','--session','xweb-swift-01a08b0a','--profile','Default','--headed','--pin-tab']
def run(*args):
...
def visit(name,w,phase='baseline'):
 ...
 run('set','viewport',str(w),'1080' if w==1440 else '812');snap()
 run('open','http://127.0.0.1:4173/'+name+'/');snap();run('webmcp','list');subprocess.run(['node','/private/tmp/xweb-arto-0910/reload.mjs'],check=True);run('wait','--fn','document.fonts.check("400 16px Nimbus")');snap();run('scroll','up','20000');snap();js('document.querySelector("#lindy-cos-root")?.style.setProperty("display","none")')
 subprocess.run(['node','/private/tmp/xweb-arto-0910/capture.mjs',str(out/f'{name}-{w}.png'),'full'],check=True)
 if w==1440:subprocess.run(['node','/private/tmp/xweb-arto-0910/capture.mjs',str(R/'gallery/assets'/f'{name}.png')],check=True)
