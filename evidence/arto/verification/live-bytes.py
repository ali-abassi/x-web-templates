...
r=Path('/Users/aliabassi/Documents/ChatGPT/AI Designer/x-web-templates');expected=json.loads((r/'evidence/arto/final/deployment-input.json').read_text());results=[]
def check(item):
...
(r/'evidence/arto/final/live-bytes.json').write_text(json.dumps({'origin':'https://x-web-templates.pages.dev','time':time.time(),'results':results},indent=2));failed=[x for x in results if not x['match']];print({'checked':len(results),'matched':len(results)-len(failed),'failed':failed});assert not failed
