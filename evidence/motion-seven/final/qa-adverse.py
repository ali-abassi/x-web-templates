...
for name in ['grid-driver','clipdock','telemetry-stack','lost-in-flight','reality-studio','direct-cta','quiet-presets','']:
 for width in [1440,375]:
  ...
  out.append({'name':name or 'gallery','width':width,**stats});(O/f'{mode}-results.json').write_text(json.dumps(out,indent=2));run('screenshot',str(O/f'{name or "gallery"}-{mode}-{width}.png'))
