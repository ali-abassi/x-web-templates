...
A nature-led creative-studio page with project filters and a local inquiry draft.
...
In the source repository, the CSS and JS under `templates/bloop/dist/` are the brand-specific source. Shared primitives live in `shared/`. Run `npm run build` at the repository root before previewing or exporting. The build combines them into the independent `dist/bloop/` site and ZIP; edit source, not the root generated output. The downloaded ZIP already contains that complete combination.

## Behavior and integration boundary

Filter concept projects by Identity or Digital, expand each case study, and download a project inquiry. These are concept projects, not claimed clients. No message is sent.
